...
namespace GShopPriceBot.Parsers
{
    ...
    /// </summary>
    public class PriceParser_NPOI : IPriceParcer
    {
        private readonly ILogger _log;
        public PriceParser_NPOI(ILogger logger)
        {
            if (logger == null)
                throw new ArgumentNullException(nameof(logger));

            _log = logger.ForContext<PriceParser_NPOI>();
        }
        ...
        public PricePosition[] Parse(Stream stream)
        {
            try
            {
                ...
                var currencyRate = string.IsNullOrWhiteSpace(priceLine)
                        ? 1m
                        : decimal.Parse(Regex.Match(priceLine, "[0-9]+").Value);
                ...
                var result = Enumerable.Empty<PricePosition>()
                    ...
                    .Concat(ParseColumn(sheet, 3, currencyRate))
                    .Concat(ParseColumn(sheet, 6, currencyRate, false))
                    .Concat(ParseColumn(sheet, 8, currencyRate))
                    .ToArray();
        ...
        /// <returns></returns>
        private IEnumerable<PricePosition> ParseColumn(ISheet sheet, int columnOffset, decimal currencyRate, bool includeBundlePrice = true)
        {
