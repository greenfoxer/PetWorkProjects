...
{
    /// <summary>
    /// Sqlite price storage (version 2, added special offer price)
    /// </summary>
    public class SqlitePriceStorage_V2 : IPriceStorage
    {
#if DEBUG
        private static readonly string _connectionString = @"Data Source=prices.db3";
#else
        private static readonly string _connectionString = @"Data Source=/app/data/prices.db3";
#endif

        private readonly ILogger _log;
        public SqlitePriceStorage_V2(ILogger log)
        {
            if (log == null)
                throw new ArgumentNullException(nameof(SqlitePriceStorage));

            _log = log.ForContext<SqlitePriceStorage_V2>();
        }
        ...
        public IEnumerable<PricePosition> Find(string namePart, string category, int limit)
        {
            var query =
                $@"SELECT 
                    ...
                    , `{nameof(PricePosition.SpecialOfferPrice)}`
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        result.Add(new PricePosition
                        {
                            ...
                            SpecialOfferPrice = reader.GetDecimal(3),
                            Category = reader.GetString(4),
                            SubCategory = reader.GetString(5),
                            CurrencyRate = reader.GetDecimal(6),
                            IsPriceChanged = reader.GetBoolean(7),
        ...
        public void Init()
        {
            ...
            var createPricePositionsTableQuery =
                $@"CREATE TABLE IF NOT EXISTS `{nameof(PricePosition)}` (
                    ...
                    `{nameof(PricePosition.SpecialOfferPrice)}` REAL NOT NULL,
